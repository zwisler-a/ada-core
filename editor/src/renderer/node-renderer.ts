...
export class NodeRenderer {
    ...
    private renderNode(node: NodeInstanceDto, cam: Camera) {
        ...
        const descriptionMetrics: TextMetrics = this.ctx.measureText(node.description);

        this.ctx.font = `${rowTextSize}px Arial`;
        const attributeMetrics: TextMetrics[] = node.attributes?.map(o => this.ctx.measureText(o.name)) ?? [];
        const inputMetrics: TextMetrics[] = node.inputs?.map(o => this.ctx.measureText(o.name)) ?? [];
        const outputMetrics: TextMetrics[] = node.outputs?.map(o => this.ctx.measureText(o.name)) ?? [];
        ...
        const identifiable = [...node?.attributes ?? [], ...node?.inputs ?? [], ...node?.outputs ?? []];
